...
This page covers two audiences. [Testing your forms](#testing-your-forms) is the section for the
tests you write over a form you built. Everything else is this repo's own suite, the commands below
included: its two tiers, what each one covers, and the gate a release has to pass.
...
|---|---|---|
| `IFormidableFocusService` | `FormidableSummary` injects it outright, so a form rendering a summary needs *something* there. Register this one first. | Which field a focus move landed on — a blocked submit makes one under `FocusFirstErrorOnInvalidSubmit`, unless something says otherwise. |
| `IFormidableDomValueSync` | `FormidableInputNumber` and `FormidableInputDate` both inject it and call it on blur. | The blur-time write back to the element. |
| `IFormidableFieldOrderService` | `FormidableForm` asks it for the document order its summary lists issues in. | An order stated without a document, and with it both the issue order and the field a blocked submit focused. |

`FormidableForm` asks for a reading order after any render that changed its registered fields, and
after one a browser-side observer reports moved the existing elements around.

The order seam speaks fields rather than element ids in both directions, so a double states its
answer in the same currency the assertions are written in:
...
front of you reaches:

| Call | Reached when | Planned with |
|---|---|---|
| `orderFields` | the order resolve itself runs | `Setup<IReadOnlyList<string>>` — it hands back the ordered ids |
| `focusField` | a blocked submit, a summary-entry click, `FormidableForm`'s own error-carrying `ApplyServerIssues`, or a page calling `FocusFirstErrorAsync()` moves focus | `Setup<bool>` — did the element take focus? |
| `registerClickRecovery` | the form establishes the displaced-click guard | `Setup<bool>` — did the script find an element to scope the guard to? |
| `releaseClickRecovery` | that guard is released | `SetupVoid` |
| `observeLayout` | the form establishes the browser-side layout observer beside the resolve | `SetupVoid` |
...
The interfaces are what Formidable holds still for consumers. A double written against
`IFormidableFocusService`, `IFormidableFieldOrderService` or `IFormidableDomValueSync` states what
