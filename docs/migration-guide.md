...
- **Rules that assumed validation runs once.** Check any rule, or any handler reacting to
  validation results, that counted on there being exactly one check over the whole form (a
...
- **Submit-only rules move to the Submit profile.** A completeness rule that only makes sense at
  ...
  The bucket is not what decides when the rule speaks on screen. Live checking evaluates both buckets
