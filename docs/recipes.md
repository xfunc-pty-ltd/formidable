...
people actually want onto those switches, then gives symptoms their own lookup table. Every
recipe opens with what to set and links to the doc that explains it in full and the sample page
that demonstrates it.

## Part 1 — I want to…

### 1. I want to validate while typing, on blur, or only at submit
...
**Read:** [`docs/profiles.md`](profiles.md), [`docs/options.md`](options.md).
...
### 2. I want presence rules to wait for submit while formats answer live
...
this value present?" — two axes, so one mistake never produces two messages.
...
a `FormidableField` (kit inputs also append the `Pending` class on their own). Add
`UpdateOn="InputUpdateMode.OnInput"` for a check that answers as the user types.
...
**Read:** [`docs/async-validation.md`](async-validation.md), [`docs/options.md`](options.md)
(`RefreshDebounce`), [`docs/css-and-accessibility.md`](css-and-accessibility.md) (`Pending`).
...
### 4. I want to validate on the server and show its verdict
...
`Form.Engine.ApplyServerIssues(...)` — the errors land on the same fields.
...
**Read:** [`docs/server-integration.md`](server-integration.md), [`docs/severity.md`](severity.md).
...
### 5. I want to reveal fields conditionally without losing their rules
...
submittable. If it is decided by *UI state* alone, leave the rule unconditional: visibility is
...
**Read:** [`docs/disclosure.md`](disclosure.md), [`docs/options.md`](options.md).
...
### 6. I want to advise without blocking
...
### 7. I want every summary entry to land somewhere
...
`id="@field.ElementId"` from a `FormidableField` context, or `FormidableFieldId.For(field)`
computed in the code-behind for a native input.
...
**Read:** [`docs/css-and-accessibility.md`](css-and-accessibility.md),
...
### 8. I want to use a native or third-party control
...
`MarkTouched()`. Reach for `FieldAnchor` instead when the control already notifies the
`EditContext` itself, as every native `InputBase` descendant does, and only needs registering.
...
**Read:** [`docs/component-kit.md`](component-kit.md), [`docs/disclosure.md`](disclosure.md),
...
`ConfigureProfiles()` via `Profile(name, ...)`. Compose a selection with
...
**Read:** [`docs/profiles.md`](profiles.md), [`docs/options.md`](options.md),
[`docs/server-integration.md`](server-integration.md).
**Sample:** [`/custom-profiles`](../samples/Formidable.Sample/Pages/CustomProfiles.razor).

### 10. I want localized messages
...
your own text, pass a message *factory* (`WithMessage(_ => Resources.Something)`) so the resource
lookup happens each time the rule runs and follows the culture in force then.
...
**Read:** [`docs/profiles.md`](profiles.md) (localization and display names).
